function [xs,gs,xms,gms] = hyp_case6c1_plume(xCs,tCs,gCs,M,gamma,Nf,Ns,t,gs,gmin)
    % Calculate plume shape for case 6c1
    ...
    elseif t2c<t && t<=t2d
        ...
        if ~isempty(xRms(gRms<gm))
            % All shorter waves than current shock height are now part of the shock
            xRms(gRms<gm) = max([xRms(gRms<gm);max(xRs(gRms<gm))*ones(size(gRms(gRms<gm)))]);
        end
    ...
    elseif t2d<t && t<=t4a
        ...
        % Current location of the right front ...
        xRms = -(M./(gRms.^2)) + (1*(gRms<=g_s) + (1/(1-gamma))*(gRms> g_s)).*(-(1/(M-1))*Ns+(M*Nf+(M/(M-1))*Ns)./(gRms.^2))*(t-1);
        % ... except left-moving waves ...
        xRms(gRms<g_s) = (M./(gRms(gRms<g_s).^2));
        % ... except right-moving waves that were eaten by the peak ...
        gp_ts = gRms(g2d<gRms);
        ...
        gm0 = g2c;
        tm0 = t2c;
        gm_ts = gRms(logical((g_0<gRms).*(gRms<=g2d)));
        tms = hyp_shock(gamma,c1,c2,c3,c4,gm0,tm0,gm_ts);
        xm_ts = (M./(gm_ts.^2)) + (-(1/(M-1))*Ns+(M*Nf+(M/(M-1))*Ns)./(gm_ts.^2)).*(tms-1);
        xRms(logical((g_0<gRms).*(gRms<=g2d))) = xm_ts;
        ...
        gm0 = g2d;
        tm0 = t2d;
        gm_ts = gLms(logical((gm<=gLms).*(gLms<g2d)));
        tms = hyp_shock(gamma,c1,c2,c3,c4,gm0,tm0,gm_ts);
        xm_ts = -(M./(gm_ts.^2)) + (1/(1-gamma))*(-(1/(M-1))*Ns+(M*Nf+(M/(M-1))*Ns)./(gm_ts.^2)).*(tms-1);
        xRms(logical((gm<=gLms).*(gLms<g2d))) = max([xRms(logical((gm<=gLms).*(gLms<g2d)));xm_ts]);
        if ~isempty(xm_ts)
    ...
    elseif t4a<t && t<=t4b
        ...
        gm0 = g2d;
        tm0 = t2d;
        gm_ts = gLms(logical((g_0<=gLms).*(gLms<g2d)));
        tms = hyp_shock(gamma,c1,c2,c3,c4,gm0,tm0,gm_ts);
        xm_ts = -(M./(gm_ts.^2)) + (1/(1-gamma))*(-(1/(M-1))*Ns+(M*Nf+(M/(M-1))*Ns)./(gm_ts.^2)).*(tms-1);
        xRms(logical((g_0<=gLms).*(gLms<g2d))) = max([xRms(logical((g_0<=gLms).*(gLms<g2d)));xm_ts]);
        if ~isempty(xm_ts)
            xRms(gRms<g4a) = max([max(xm_ts)*ones(size(gRms(gRms<g4a)));xRms(gRms<g4a)]);
        end
    ...
    elseif t4b<t
        ...
        gm0 = g2d;
        tm0 = t2d;
        gm_ts = gLms(logical((g4a<=gLms).*(gLms<g2d)));
        tms = hyp_shock(gamma,c1,c2,c3,c4,gm0,tm0,gm_ts);
        xm_ts = -(M./(gm_ts.^2)) + (1/(1-gamma))*(-(1/(M-1))*Ns+(M*Nf+(M/(M-1))*Ns)./(gm_ts.^2)).*(tms-1);
        xRms(logical((g4a<=gLms).*(gLms<g2d))) = max([xRms(logical((g4a<=gLms).*(gLms<g2d)));xm_ts]);
        if ~isempty(xm_ts)
            xRms(gRms<g4a) = max([max(xm_ts)*ones(size(gRms(gRms<g4a)));xRms(gRms<g4a)]);
        end
